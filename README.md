...
8. Rename the `.env_example` file to `.env`.
9. Open the `.env` file and change the `MODS_DIR` and `USER_PROFILE` variables to the correct paths.
...
10. Replace the `user_profile.cfg` file in the `USER_PROFILE` path with the one provided in the repository.
...
12. Subscribe to the mods you want on mod.io.
13. Run the `ModIO_SR.ps1` file (you can run it by right clicking and selecting run with powershell) and wait until all the mods you subscribed to are downloaded.
